...
int main()
{
    ...
    initCanvas("Mathematics Scrabble",1000,700);
    Rectangle cvr1(500,350,1000,700);
    cvr1.setFill(true);
    cvr1.setColor(COLOR(40,0,0));

start1:

    beginFrame();
    Rectangle r_title(500,150,250,75);
    Text t_title(500,150,"Mathematics Scrabble");
    Rectangle r_start(500,300,100,50);
    Text t_start(500,300,"New Game");
    Rectangle r_about(500,450,100,50);
    Text  t_about(500,450,"About");
    Rectangle r_instruction(500,600,100,50);
    Text t_instruction(500,600,"Instruction");
    Rectangle r_exit(500,660,50,25);
    Text t_exit(500,660,"Exit");
    r_exit.setFill(true);
    r_exit.setColor(COLOR(255,255,204));
    r_title.setFill(true);
    r_title.setColor(COLOR(255,255,0));
    r_start.setFill(true);
    r_start.setColor(COLOR(255,255,204));
    r_about.setFill(true);
    r_about.setColor(COLOR(255,255,204));
    r_instruction.setFill(true);
    r_instruction.setColor(COLOR(255,255,204));
    computer_score=0;
    player1_score=0;
    endFrame();
...
start2:
    ...
    int user=getClick();
    int userx=user/65536;
    int usery=user%65536;
...
//user sees about;
    if(rectangle_detect(userx,usery,500,450,100,50))
    {
        ...
        r_exit.hide();
        t_exit.hide();
        Rectangle r_aboutagain(500,75,120,40);
        r_aboutagain.setFill(true);
        r_aboutagain.setColor(COLOR(255,255,204));
        Text t_about(500,75,"About the game");
        Rectangle r_text_cover(400,320,750,200);
        r_text_cover.setFill(true);
        r_text_cover.setColor(COLOR(255,255,204));
        ...
        Text t_3rd(380,340,"3. Player can pass or exchange and pass at his will, if he thinks no possible equation can be set up.");
        ...
        r_startagain.setFill(true);
        r_startagain.setColor(COLOR(255,255,204));
        Text t_startagain(500,650,"Start");
        Rectangle r_back1(120,650,100,50);
        r_back1.setFill(true);
        r_back1.setColor(COLOR(255,255,204));
        Text t_back1(120,650,"Back");
        endFrame();
start:
        int useragain =getClick();
        if(rectangle_detect(useragain/65536,useragain%65536,500,650,100,50))
        {
            start_game=1;
        }
        else
        {
            if(rectangle_detect(useragain/65536,useragain%65536,120,650,100,50))
            {
                goto start1;
            }
            else
...
//user sees instructions;
    else if( rectangle_detect(userx,usery,500,600,100,50))
    {
        ...
        r_exit.hide();
        t_exit.hide();
        Rectangle r_cover2(500,100,100,50);
        r_cover2.setFill(true);
        r_cover2.setColor(COLOR(255,255,204));
        Rectangle r_text_cover(425,320,725,240);
        r_text_cover.setFill(true);
        r_text_cover.setColor(COLOR(255,255,204));
        Text t_instruction1(500,100,"Instructions");
        Text t_1st(420,260,"1. Click on the tiles and then drag them to the correct position on the board to make equation.");
        Text t_2nd(410,300,"2. Player should not string together arbitrary symbols. example: '3+xx4==12' is NOT valid.");
        Text t_3rd(365,340,"3. Order of operation:The game will function on a strict left-to-right evaluation.");
        Text t_4th(390,380,"4. Press SUBMIT to submit ,PASS to pass the turn and EXCHANGE to exchange tiles.");
        Text t_5th(295,420,"5. For further elaborated instructions refer 'readme.txt'.");
        Rectangle r_startagain(500,650,100,50);
        r_startagain.setFill(true);
        r_startagain.setColor(COLOR(255,255,204));
        Text t_startagain(500,650,"Start");
...
start3:
    ...
    else if(rectangle_detect(userx,usery,500,680,50,25))
    {
        return 0;
    }
...
//user starts the game
    else if(rectangle_detect(userx,usery,500,300,100,50))
    {
        start_game=1;
    }
...
//board using 15 rectangles;
        ...
        beginFrame();
        Rectangle cvr(500,350,1000,700);
        cvr.setFill(true);
        cvr.setColor(COLOR(40,0,0));
        Rectangle r[16][16],r_back[16][16];
        Text t[16][16];
        ...
        for(int i=1; i<=15; i++)
        {
            for(int j=1; j<=15; j++)
            {
                r[i][j]=Rectangle(165+30*i,125+30*j,30,30);
                r_back[i][j]=Rectangle(165+30*i,125+30*j,30,30);
...
//
                ...
                {
                    r[i][j].setColor(COLOR(0,0,255));
                    t[i][j]= Text(center_on_board_x[i][j],center_on_board_y[i][j],"x3");
                }


                if((i==j||(i==16-j))&&(i>1)&&(i<15)&&(j>1)&&(j<15))
                {
                    r[i][j].setFill(true);
                    r[i][j].setColor(COLOR(255,0,0));
        ...
        Rectangle r2[2],r_result;
        r2[0]= Rectangle(60,200,80,60);
        r2[1]= Rectangle(60,200,80,60);
        Text t2(60,200,"PASS"),t_result;
        r2[0].setFill(true);
        r2[0].setColor(COLOR(255,255,204));
        Rectangle r3[2];
        r3[0]= Rectangle(60,320,80,60);
        r3[1]= Rectangle(60,320,80,60);
        Text t3(60,320,"EXCHANGE");
        r3[0].setFill(true);
        r3[0].setColor(COLOR(255,255,204));
        Rectangle r4[2];
        r4[0]= Rectangle(60,440,80,60);
        r4[1]= Rectangle(60,440,80,60);
        Text t4(60,440,"SUBMIT");
        r4[0].setFill(true);
        r4[0].setColor(COLOR(255,255,204));
        Rectangle r5[2];
        r5[0]= Rectangle(60,660,80,60);
        r5[1]= Rectangle(60,660,80,60);
        Text t5(60,660,"QUIT");
        r5[0].setFill(true);
        r5[0].setColor(COLOR(255,255,204));

        Rectangle r12[2];
        r12[0]=Rectangle(800,200,180,40);
        r12[1]=Rectangle(800,200,180,40);
        Text t12(760,200,"Computer");
        r12[0].setFill(true);
        r12[0].setColor(COLOR(255,255,204));

        Rectangle r13[2];
        r13[0]=Rectangle(800,280,180,40);
        r13[1]=Rectangle(800,280,180,40);
        Text t13(760,280,"Player");
        r13[0].setFill(true);
        r13[0].setColor(COLOR(255,255,204));
...
// VARIABLES FOR TRANSITION EFFECT
        ...
        Circle c1[2];
        c1[0]=Circle(405,65,44);
        c1[1]=Circle(405,65,44);
        Text t14(in_x,65,"Player"), t15(out_x,out_y,"Computer"),t16(850,280,"0"),t17(850,200,"0");
        c1[0].setFill(true);
        c1[0].setColor(COLOR(255,255,204));


        endFrame();

        Rectangle r_tiles_player[7],r_over[7];
        Text t_tiles_player[7];
        bool detection[7];
        int center_rectangle_x[7],center_rectangle_y[7];
        bool exch=0,submit=0,pass=0,quit=0;
...
//check for endgame;
            if ( current== last)
            {
                if (computer_score>player1_score)
                {
                    ...
                    r_result.setFill(true);
                    r_result.setColor(COLOR(255,255,204));
                    endFrame();
                }

                else if(computer_score<player1_score)
                {
                    beginFrame();
                    r_result=Rectangle(800,600,100,50);
...
//player1.tiles
            ...
            for(int i=0; i<7; i++)
            {
                r_tiles_player[i]=Rectangle(300+30*i,630,30,30);
                r_over[i]=Rectangle(300+30*i,630,30,30);
                t_tiles_player[i]=Text(300+30*i,630,player1[i].tile_display);
                r_tiles_player[i].setFill(true);
                r_tiles_player[i].setColor(COLOR(255,255,204));
            }
...
//copying and feding into array;
                ...
                if(quit)
                {
                    ...
                    r_thank.setFill(true);
                    r_thank.setColor(COLOR(255,255,204));
                    endFrame();
...
//PRECONDITIONS satisfaction
            ...
            for(int i=0; i<7; i++)
            {
                ...
                for(int j=1; j<16; j++)
                {
                    for(int k=1; k<16; k++)
                    {
                        if(rectangle_detect(center_rectangle_x[i],center_rectangle_y[i],center_on_board_x[j][k],center_on_board_y[j][k],30,30))
                        {
                            t[j][k]=t_tiles_player[i];
                            t[j][k].imprint();
...
// check for endgame;
            ...
            case 0 :
            {
                int index_comp= -1;               //exchange
                 beginFrame();
                    Rectangle r_popup;
                    Text t_popup;
                    r_popup=Rectangle(800,600,200,45);
                    t_popup=Text(800, 600,"Computer exchanged a tile!!");
                    r_popup.setFill(true);
                    r_popup.setColor(COLOR(255,255,204));
                    endFrame();
                    wait(2);
                    beginFrame();
                    r_popup.hide();
            ...
            case 1 :
            {
                for(int i=0; i<17; i++)
                {
                    for(int j=0; j<17; j++)
                    {
                        if(temp_board[i][j]!='a'&&last_board[i][j]=='a')
                        {
                            beginFrame();
